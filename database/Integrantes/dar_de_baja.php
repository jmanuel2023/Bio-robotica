...
include '../Conexion.php';
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['id'])) {
        ...
        $motivo = $_POST['motivo'];
        $select_sql = "SELECT correo, nombre, nombre_imss, nombre_ine, nombre_credencial, nombre_domicilio, nombre_horario FROM integrantes WHERE id = $id";
        ...
        if ($result && $result->num_rows > 0) {
            ...
            foreach ($documentos as $documento) {
                $ruta_documento = '../../documents/' . $documento .'.pdf';
                if (file_exists($ruta_documento)) {
                    unlink($ruta_documento);
                }
            }
        }
        $delete_sql = "DELETE FROM integrantes WHERE id = $id";
            if ($conn->query($delete_sql)) {
                ...
                try {
                    ...
                    $mail->Subject = '¡Lamentamos esto!';
                    $mail->Body = "Hola $nombre,\n\n Lamentamos informarte que ya no formas parte del club de Bio-Robotica \n\n Observaciones: \n\n $motivo \n\n Si crees que es un error favor de contestar a este correo o acudir al salon 1008.";
                    // Adjuntar la imagen
